...
export class EmployeeRestService {
  constructor(protected proxy: ProxyService, protected appConfigService: AppConfigService) {}
  ...
  createEmployee(data: EmployeeModel): Observable<DataResponse<EmployeeModel>> {
    ...
    return this.proxy.post<DataResponse<EmployeeModel>>(url, body, {
  ...
  updateEmployee(data: EmployeeModel): Observable<DataResponse<EmployeeModel>> {
    ...
    url = `${url}/${data._id}`;
    const body = JSON.stringify(classToPlain(data));
    return this.proxy.put<DataResponse<EmployeeModel>>(url, body, {
      headers: { 'Content-type': ContentType.json },
    });
  }
  ...
  uploadAvatar(file: File): Observable<any> {
    if (file instanceof File) {
      ...
      return this.proxy.post('/uploadFiles', formData).pipe(map((res) => res));
    }
